...
Prior work [7] has shown that attention mechanism has helped a lot in Natural language understanding task. Also previous work [6] claims that training Intent classifier using BERT [5] has achieved significant improvement on intent classification accuracy . Considering which an optimal choice to train the intent classification was BERT.
